...
export function createFileEditTool(policy: ApprovalPolicy) {
  return tool({
    name: 'file_edit',
    ...
    execute: async ({ path, edits }) => {
      if (needsApproval(policy, escapesCwd(path))) {
        const ok = await confirm('file_edit', path, `edit: ${path}`);
        if (!ok) return { error: 'Denied by user. Do not retry this edit.' };
      }
