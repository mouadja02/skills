...
- smoke-test a server with the FastMCP CLI before wiring it into an MCP client
...
Use the client's MCP configuration workflow when the server already exists and only needs to be connected. Use ad-hoc MCP CLI tooling when the goal is direct command-line access to an existing MCP server instead of building one.
...
```bash
python <installed-skill-dir>/scripts/scaffold_fastmcp.py \
...
```bash
python <installed-skill-dir>/scripts/scaffold_fastmcp.py --list
```
...
When the goal is client integration, either:

- configure the server in the target client's MCP configuration, or
...
### Client cannot see the deployed server

The server-building part may be correct while the client config is not. Re-check the MCP server command, environment variables, transport URL, and client restart requirements.
...
Imported from NousResearch Hermes Agent optional skills (optional-skills/mcp/fastmcp) under the MIT license. Source: https://github.com/NousResearch/hermes-agent/tree/main/optional-skills/mcp/fastmcp
