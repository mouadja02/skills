...
```
$SKILLS_HOME/personal-productivity/memento-flashcards/data/cards.json
```

If `SKILLS_HOME` is not set, the helper script defaults to `~/.skills/personal-productivity/memento-flashcards/data/cards.json`. Set `MEMENTO_CARDS_FILE` to override the exact file path.
...
```bash
python3 <installed-skill-dir>/scripts/memento_cards.py add \
...
```bash
python3 <installed-skill-dir>/scripts/memento_cards.py due
```

This returns a JSON array of cards where `next_review_at <= now`. If a collection filter is needed:

```bash
python3 <installed-skill-dir>/scripts/memento_cards.py due --collection "History"
```
...
```bash
python3 <installed-skill-dir>/scripts/memento_cards.py rate \
...
```bash
python3 <installed-skill-dir>/scripts/youtube_quiz.py fetch VIDEO_ID
```
...
```bash
python3 <installed-skill-dir>/scripts/memento_cards.py add-quiz \
  --video-id "VIDEO_ID" \
...
**Export:**
```bash
python3 <installed-skill-dir>/scripts/memento_cards.py export \
...
**Import:**
```bash
python3 <installed-skill-dir>/scripts/memento_cards.py import \
  --file ~/flashcards.csv \
  --collection "Imported"
```
...
```bash
python3 <installed-skill-dir>/scripts/memento_cards.py stats
```
...
```bash
python3 <installed-skill-dir>/scripts/memento_cards.py stats
python3 <installed-skill-dir>/scripts/memento_cards.py add --question "Capital of France?" --answer "Paris" --collection "General"
python3 <installed-skill-dir>/scripts/memento_cards.py due
```
...
Imported from NousResearch Hermes Agent optional skills (optional-skills/productivity/memento-flashcards) under the MIT license. Source: https://github.com/NousResearch/hermes-agent/tree/main/optional-skills/productivity/memento-flashcards
