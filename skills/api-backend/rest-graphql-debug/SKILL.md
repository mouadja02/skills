...
Drive REST and GraphQL diagnosis through shell commands for `curl`, short Python scripts for `requests`, and current vendor docs when available. Isolate the failing layer before guessing at the fix.
...
### Python (requests) via Python script
...
Drop this into `tests/` and run `pytest tests/test_api_smoke.py -v`:
...
- Never hardcode tokens in scripts. Read from environment variables such as `os.environ["API_TOKEN"]` or a local `.env` file that stays out of git.
...
### Python — for multi-step flows

When debugging spans auth → fetch → paginate → validate, use a short Python script. Print concise status lines and keep full tokens out of logs:
...
Pull the spec for the endpoint you're debugging instead of guessing. Use the available browser, web search, or documentation lookup tool for the current environment.

For larger CRUD sweeps, run a focused test script or delegate only when the current agent environment provides an explicit subagent/delegation mechanism.
...
Imported from NousResearch Hermes Agent optional skills (optional-skills/software-development/rest-graphql-debug) under the MIT license. Source: https://github.com/NousResearch/hermes-agent/tree/main/optional-skills/software-development/rest-graphql-debug
